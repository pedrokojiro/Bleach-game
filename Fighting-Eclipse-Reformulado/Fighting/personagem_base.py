...
class PersonagemBase:
    def __init__(self, x, y, perfil, identidade):
        self.perfil, self.identidade = perfil, identidade
        ...
        self.moves = golpes(identidade,perfil.dano,perfil.alcance)
    ...
    def dash(self):
        if not self.pode_agir() or not self.consumir_stamina(23):return False
        self.estado(E.DASH,.17)
        self.velocidade_x = (self.movimento or self.direcao)*850
        self.invulnerabilidade = .12 if self.identidade==2 else .045
    ...
    def ativar_liberacao(self):
        if not self.pode_agir() or self.forma_liberada or self.exaustao or not self.consumir_reiatsu(70):return False
        self.forma_liberada,self.tempo_liberacao = True,10.
        self.invulnerabilidade = .7
        self.estado(E.RELEASE,.7)
        self.anunciar('LIBERAÇÃO')
        self.criar_efeito('release')
        return True

    def desativar_liberacao(self):
        self.forma_liberada = False
        self.tempo_liberacao,self.exaustao = 0,3.
    ...
    def obter_hitbox(self):
        ...
        reach = self.acao.alcance * (1.25 if self.forma_liberada else 1)
    ...
    def receber_dano(self, move, atacante, projetil=False):
        ...
        if self.estado_atual==E.PARRY and self.tempo_estado>(.14 if self.identidade==3 else .21) and move.tipo!='grab' and frontal:
        ...
        damage = move.dano*scale*(1-self.defesa)*(1.14 if atacante.forma_liberada else 1)
        if blocked:
            self.stamina = max(0,self.stamina-move.dano*.8)
            damage *= .15
            self.estado(E.BLOCKSTUN,.16)
            if self.identidade==3:self.ganhar_reiatsu(7)
            if self.stamina<=0:
                self.estado(E.HITSTUN,.7); self.anunciar('GUARDA QUEBRADA')
            self.criar_efeito('block')
        else:
            armor = self.identidade==1 and self.acao and self.acao.estado==E.ATTACK_HEAVY and self.tempo_acao<self.acao.startup+self.acao.active
            if self.barreira>0:damage *= .55
            if self.forma_liberada and self.identidade==3:damage *= .72
            ...
            atacante.ganhar_reiatsu(9 if atacante.identidade==0 else 6)
    ...
    def atualizar(self, dt, outro):
        ...
        self.stamina = min(100,self.stamina+dt*(7 if self.guarda else 20))
    ...
    def atualizar_fisica(self,dt):
        if self.pode_agir():
            speed = self.velocidade_movimento*(1.3 if self.correndo else 1)*(1.18 if self.forma_liberada else 1)*(.72 if self.exaustao else 1)
