...
VISUALS = {
 'swordsman': ((15,20,30),(100,210,250),142,24),
 'beast': ((75,31,38),(250,104,84),166,39),
 'archer': ((27,56,60),(110,190,255),138,18),
 'guardian': ((43,37,63),(190,159,250),151,32),
 'human': ((62,52,38),(255,197,102),140,21),
}
...
def sprite(identity:int,state:E,frame:int,released:bool,direction:int):
    ...
    if isinstance(identity,int):
        ...
        identity=PERFIS[identity].id
    identity=KITS[identity].visual
    s=pygame.Surface((300,230),pygame.SRCALPHA)
    dark,accent,height,breadth=VISUALS[identity]
    ...
    if released and identity=='archer':
        for side in (-1,1):
            poly((*accent,125),[(cx,top+50),(cx+side*105,top-10),(cx+side*62,top+83)])
    if released and identity=='beast':
        for side in (-1,1):
            poly(white,[(cx+side*28,top+50),(cx+side*62,top+15),(cx+side*48,top+78)])
    # Trailing fabric, distinct for each silhouette.
    if identity in ('swordsman','archer'):
    ...
    legw=19 if identity in ('beast','guardian') else 13
    for side in (-1,1):
        ...
        line(white if identity=='archer' else (75,79,90),(foot[0]-6,foot[1]),(foot[0]+14,foot[1]),8)
    ...
    if identity=='swordsman':
        ...
        poly(accent,[(cx-22,top+41),(cx+20,top+41),(cx+24,top+86),(cx-21,top+86)])
        line(white,(cx,top+43),(cx,top+84),3)
    else:
    ...
    if identity=='swordsman':
        ...
        poly((44,28,28),[(head[0]-18,head[1]+5),(head[0]-19,head[1]-16),(head[0]+10,head[1]-20),(head[0]+17,head[1]-5)])
        line(accent,(head[0]-19,head[1]-7),(head[0]+13,head[1]-10),4)
    else:
    ...
    line(accent if released or identity=='guardian' else ink,(head[0]+4,head[1]),(head[0]+13,head[1]-1),3)
    hand=(cx+44,top+70)
    ...
    line(ink,(shoulder[0]+breadth-2,top+45),hand,18 if identity in ('beast','guardian') else 13)
    line(dark,(shoulder[0]+breadth-2,top+45),hand,12)
    circle(skin,hand,7 if identity!='beast' else 12)
    if identity=='swordsman':
        ...
        pygame.draw.rect(s,white,(hand[0]+2,hand[1]-16,15,30))
        line(accent,(hand[0]+9,hand[1]-12),(hand[0]+9,hand[1]+10),3)
    else:
        hx,hy=hand
...
def draw_fighter(s,p):
    ...
    if p.form_index>0:
        pygame.draw.circle(s,p.cor_base,p.rect.center,100,2)
        pygame.draw.circle(s,p.cor_base,p.rect.center,110,1)
    if p.charging:
        pygame.draw.circle(s,p.cor_base,(p.rect.centerx+p.direcao*45,p.rect.centery),8+int(p.charge_time*22),2)
...
def portrait(identity,size=100):
    ...
    if isinstance(identity,int):
        from seu_personagem import PERFIS
        identity=PERFIS[identity].id
    top=211-VISUALS[KITS[identity].visual][2]
